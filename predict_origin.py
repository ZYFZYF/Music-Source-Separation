...
PREFIX = os.path.splitext(os.path.split(__file__)[1])[0] + '_'
...
def judge(input, output, predict):
    # 每个堆叠沙漏网络的输出的loss和
    return sum(torch.mean(torch.abs(predict[x] - output)) for x in range(NUM_STACKS))
...
def train():
    ...
    for i in tqdm.tqdm(range(MAX_ITERATIONS)):
        ...
        print(torch.sum(input), torch.sum(output), torch.sum(predict), loss)
        loss.backward()
        loss_sum += loss
        optimizer.step()
        if (i + 1) % TRAIN_SAVE_POINT == 0:
            torch.save(net.state_dict(), 'Model/{}checkpoint_{}.pt'.format(PREFIX, i + 1))
            print("loss of {} is {}".format(i + 1, loss_sum / TRAIN_SAVE_POINT))
            loss_sum = 0
    torch.save(net.state_dict(), 'Model/{}checkpoint_final.pt'.format(PREFIX))
    print("-------------------end training.....----------------------")
...
def test():
    ...
    # TODO 这里设成输出为2通道，可以尝试输出一通道效果如何，同时也得更改loss计算方式
    net = get_model()
    net.load_state_dict(torch.load('Model/{}checkpoint_final.pt'.format(PREFIX)))
    net.to(device)
    ...
    for left_origin, right_origin, mix_origin, left_mag, right_mag, mix_mag, max_value, mix_spec_phase in Utils.mir_1k_data_generator(
        ...
        predict_left = predict_left * max_value
        predict_right = predict_right * max_value
        print(np.sum(predict_left))
        print(np.sum(predict_right))
        ...
        if cnt % TEST_STEP == 0:
            ...
            Utils.write_wav(predict_left_wav,
                            'Samples/{}{}_accompaniments_predict.wav'.format(PREFIX, cnt // TEST_STEP))
            Utils.write_wav(predict_right_wav, 'Samples/{}{}_voice_predict.wav'.format(PREFIX, cnt // TEST_STEP))
            Utils.write_wav(left_origin, 'Samples/{}{}_accompaniments_origin.wav'.format(PREFIX, cnt // TEST_STEP))
            Utils.write_wav(right_origin, 'Samples/{}{}_voice_origin.wav'.format(PREFIX, cnt // TEST_STEP))
            Utils.write_wav(mix_origin, 'Samples/{}{}_mixed.wav'.format(PREFIX, cnt // TEST_STEP))
        if cnt == TOTAL_TEST:
            break
    print('人声GNSDR={:.3f} 人声GSIR={:.3f} 人声GSAR={:.3f} 伴奏GNSDR={:.3f} 伴奏GSIR={:.3f} 伴奏GSAR={:.3f}'.format(
        (gnsdr / totalLen)[0][0],
